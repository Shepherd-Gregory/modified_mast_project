...
//        typedef g2o::BlockSolverTraits<Eigen::Dynamic, Eigen::Dynamic> Block;
//        Block::LinearSolverType* solver = new g2o::LinearSolverCSparse<Block::PoseMatrixType();
//        Block* blockSolver=new Block(unique_ptr<Block::LinearSolverType>(solver));

//        std::unique_ptr<Block::LinearSolverType> solver (new g2o::LinearSolverCSparse<Block::PoseMatrixType());
//        std::unique_ptr<Block> blockSolver (new Block(std::move(solver)));
//        g2o::OptimizationAlgorithmLevenberg* algorithm = new g2o::OptimizationAlgorithmLevenberg(unique_ptr<Block>(blockSolver));
    ...
    void SFMgraph::AddImageandSonarEdges(JPL7 *Camera_vertex, JPL7 *Calib, cv::Mat Image, cv::Mat Sonar,
        ...
        for (std::size_t i = 0; i < r_theta.size(); i++) {
    ...
    void SFMgraph::find_camera_from_features(JPL7 *Camera_1, vector<Eigen::Matrix<double, 3, 1>> uv,
        ...
        vector<Feature*> Feature_list;
        for (std::size_t i = 0; i < Feature_Class_list.size(); i++) {
